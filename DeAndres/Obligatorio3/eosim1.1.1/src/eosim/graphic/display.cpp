...
namespace graphic {
    ...
    struct TextSprite::impl : boost::noncopyable {
        ...
        impl(shared_ptr<Display::impl> dis, const char* text, double size, double x, double y, double zindex)
            : dis(dis)
            , x(x), y(y)
            , visible(1)
            , caption(text)
    ...
    TextSprite::TextSprite(Display& dis, const char* text, double size, double x, double y, double zindex)
        : p_impl(new TextSprite::impl(dis.p_impl, text, size, x, y, zindex))
    {}
    ...
    void Display::impl::draw()
    {
        ...
        if(m_feng.load_font("timesi.ttf", 0, gren)) {
                ...
                ;++it)
            {
                ...
                if (tsimpl->visible && tsimpl->zindex == 0) {
                    ...
                    for (std::string::iterator letra = tsimpl->caption.begin(), last = tsimpl->caption.end(); letra != last; ++letra) {
                        ...
                        if (glyph) {
                            ...
                            rs.color(agg::rgba8(255, 255, 255));
        ...
        if(m_feng.load_font("timesi.ttf", 0, gren)) {
            ...
            {
                TextSprite::impl* tsimpl = *it;
                if (tsimpl->visible && tsimpl->zindex == 1) {
                    m_feng.height(tsimpl->size);
                    m_feng.width(tsimpl->size);
                    m_contour.width(-tsimpl->size * tsimpl->size * 0.05);
                    double x  = tsimpl->x;
